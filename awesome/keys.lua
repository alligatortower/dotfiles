...
local function tag_view_nonempty(step, s)
	...
	local tags = s.tags
	local initialIndex = s.selected_tag.index
	local index = initialIndex

	while true do
		index = index + step
		if index == initialIndex then
			break
...
local superKey = "Mod4"
local altKey = "Mod1"
local volumeMute = "XF86AudioMute"
local volumeUp = "XF86AudioRaiseVolume"
local volumeDown = "XF86AudioLowerVolume"
...
globalkeys = gears.table.join(
	...
	awful.key({ superKey, "Shift" }, "Tab", function()
		tag_view_nonempty(-1)
	end),
	...
	awful.key({ superKey, "Shift" }, "q", awesome.quit, { description = "quit awesome", group = "awesome" }),
	...
	-- audio
	awful.key({}, volumeMute, function()
		volume_widget:toggle()
	end, { description = "Mute Volume", group = "widgets" }),
	awful.key({}, volumeUp, function()
		volume_widget:inc(5)
	end, { description = "Increase Volume", group = "widgets" }),
	awful.key({}, volumeDown, function()
		volume_widget:dec(5)
	end, { description = "Decrease Volumee", group = "widgets" }),
...
clientkeys = gears.table.join(
	...
	awful.key({ superKey, "Shift" }, "c", function(c)
		c:kill()
